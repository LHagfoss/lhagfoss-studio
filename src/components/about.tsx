                ...
                <div className="w-full h-[15vw] text-thirdy gap-1 text-[3vw] leading-8 md:leading-8 text1-container flex flex-wrap overflow-hidden p-10 bg-primary rounded-[3vw]">
                    { isVisible && (
                        text1.split(" ").map((word, index) => (
                            <motion.div key={index} initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.1 + index * 0.05, ease: easeInOut }} className={`px-2 ${index === 1 || index === 2 || index === 16 ? "text-secondary" : "text-thirdy"} text-[8vw] md:text-[3vw]`}>{word}</motion.div>
                        ))
                    )}
                </div>
                        ...
                        <div className="w-full h-full flex flex-wrap">
                            { isVisible && (
                                text2.split(" ").map((word, index) => (
                                    <motion.div key={index} initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 1 + index * 0.05, ease: easeInOut }} className="px-2 py-1 text-thirdy text-[8vw] md:text-[2vw]">{word}</motion.div>
                                ))
                            )}
                        </div>
                        ...
                        <div className="w-full h-full flex flex-wrap">
                            { isVisible && (
                                text3.split(" ").map((word, index) => (
                                    <motion.div key={index} initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 2 + index * 0.05, ease: easeInOut }} className="px-2 text-thirdy text-[8vw] md:text-[2vw]">{word}</motion.div>
                                ))
                            )}
                        </div>
                        <div className="flex gap-3">
